...
find /app/.next \( -type d -name .git -prune \) -o -type f -print0 | xargs -0 sed -i "s#APP_NEXT_PUBLIC_BACKEND_BASE_URL#$NEXT_PUBLIC_BACKEND_BASE_URL#g"
...
find /app/.next \( -type d -name .git -prune \) -o -type f -print0 | xargs -0 sed -i "s#APP_NEXT_PUBLIC_CDN_BASE_URL#$NEXT_PUBLIC_CDN_BASE_URL#g"
